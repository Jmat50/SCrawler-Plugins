    ...
    Public Class UserData
        ...
        Public Sub GetMedia(ByVal Token As CancellationToken) Implements IPluginContentProvider.GetMedia
            ...
            If String.IsNullOrWhiteSpace(listingUrl) Then
                UserExists = False
                LogProvider?.Add("Tumblr: unable to resolve listing URL.")
                Exit Sub
            End If
            ...
            While pending.Count > 0
                ...
                Catch ex As WebException
                    ...
                    If status.HasValue AndAlso status.Value = HttpStatusCode.NotFound Then
                        ...
                        LogProvider?.Add(String.Format("Tumblr: page not found: {0}", pageUrl))
                    ElseIf status.HasValue AndAlso status.Value = HttpStatusCode.Gone Then
                        If isRootPage Then UserSuspended = True
                        LogProvider?.Add(String.Format("Tumblr: page unavailable: {0}", pageUrl))
                    Else
                        LogProvider?.Add(ex, String.Format("[Tumblr.GetMedia] failed to load page: {0}", pageUrl))
                    End If
                    Continue While
            ...
            LogProvider?.Add(String.Format("Tumblr: parsed {0} page(s), collected {1} media item(s).", pagesParsed, TempMediaList.Count))
        End Sub
        ...
        Public Sub Download(ByVal Token As CancellationToken) Implements IPluginContentProvider.Download
            ...
            For i As Integer = 0 To TempMediaList.Count - 1
                ...
                Catch ex As Exception
                    media.DownloadState = UserMediaStates.Missing
                    media.Attempts += 1
                    LogProvider?.Add(ex, String.Format("[Tumblr.Download] {0}", media.URL))
                End Try
        ...
        Public Sub DownloadSingleObject(ByVal Data As IDownloadableMedia, ByVal Token As CancellationToken) Implements IPluginContentProvider.DownloadSingleObject
            ...
            Else
                Try
                    ...
                    End If
                Catch ex As Exception
                    LogProvider?.Add(ex, String.Format("[Tumblr.DownloadSingleObject] {0}", Data.URL))
                End Try
            End If
        ...
        Private Function NormalizeToAbsoluteUrl(ByVal value As String) As String
            ...
            If Regex.IsMatch(trimmed, "^[A-Za-z0-9_.-]+\.tumblr\.com(?:/.*)?$", RegexOptions.IgnoreCase) Then
        ...
        Private Function DownloadPageText(ByVal url As String) As String
            ...
            Catch ex As WebException
                ...
                If status.HasValue AndAlso (status.Value = HttpStatusCode.Forbidden OrElse
                    ...
                    LogProvider?.Add(String.Format("Tumblr: using fallback mirror for {0}", url))
        ...
        Private Function ExtractPostLinks(ByVal html As String, ByVal pageUrl As String) As List(Of String)
            ...
            Dim relativePattern As String =
                "(?:(?:href|data-href|data-url)\s*=\s*[""'])(/(?:post/\d+(?:/[^""'<>\s]*)?|video/[A-Za-z0-9_.-]+/\d+(?:/[^""'<>\s]*)?|blog/view/[A-Za-z0-9_.-]+/\d+(?:/[^""'<>\s]*)?|[A-Za-z0-9_.-]+/\d+(?:/[^""'<>\s]*)?))"
            ...
            Dim absolutePattern As String =
                "https?://[A-Za-z0-9.-]*tumblr\.com/(?:post/\d+(?:/[^""'<>\s]*)?|video/[A-Za-z0-9_.-]+/\d+(?:/[^""'<>\s]*)?|blog/view/[A-Za-z0-9_.-]+/\d+(?:/[^""'<>\s]*)?|[A-Za-z0-9_.-]+/\d+(?:/[^""'<>\s]*)?)"
        ...
        Private Function ExtractNextPageUrl(ByVal html As String, ByVal currentPage As String) As String
            ...
            End If
            If Not m.Success Then
                m = Regex.Match(html, "href=""([^""<>]*[?&]before=\d+[^""<>]*)""", RegexOptions.IgnoreCase)
            End If
        ...
        Private Function NormalizeDirectMediaUrl(ByVal raw As String) As String
            ...
            If host = "assets.tumblr.com" Then Return String.Empty

            Dim path As String = uri.AbsolutePath
            If host.Contains("media.tumblr.com") Then
                path = Regex.Replace(path, "/s\d+x\d+(?:_[a-z0-9]+)?/", "/s1280x1920/", RegexOptions.IgnoreCase)
            End If
            ...
            Dim isVideoFileEndpoint As Boolean = path.IndexOf("/video_file/", StringComparison.OrdinalIgnoreCase) >= 0
            If Not hasKnownExtension AndAlso Not isVideoFileEndpoint Then Return String.Empty
        ...
        Private Shared Function ExtractPostIdFromUrl(ByVal value As String) As String
            ...
            If Uri.TryCreate(value, UriKind.Absolute, uri) Then
                Dim mPath As Match = Regex.Match(uri.AbsolutePath, "^/(?:post|video|image)/(\d+)(?:/|$)", RegexOptions.IgnoreCase)
                If mPath.Success Then Return mPath.Groups(1).Value

                mPath = Regex.Match(uri.AbsolutePath, "^/blog/view/[A-Za-z0-9_.-]+/(\d+)(?:/|$)", RegexOptions.IgnoreCase)
                If mPath.Success Then Return mPath.Groups(1).Value

                mPath = Regex.Match(uri.AbsolutePath, "^/[A-Za-z0-9_.-]+/(\d+)(?:/|$)", RegexOptions.IgnoreCase)
                If mPath.Success Then Return mPath.Groups(1).Value

                Dim mQuery As Match = Regex.Match(uri.Query, "(?:^|[?&])post_id=(\d+)", RegexOptions.IgnoreCase)
                If mQuery.Success Then Return mQuery.Groups(1).Value
            End If

            Dim m As Match = Regex.Match(value, "(?:/post/|/video/[A-Za-z0-9_.-]+/|/image/|/blog/view/[A-Za-z0-9_.-]+/|/[A-Za-z0-9_.-]+/)(\d{5,})", RegexOptions.IgnoreCase)
        ...
        Private Shared Function BuildMediaFileName(ByVal directUrl As String, ByVal postId As String, ByVal mediaType As UserMediaTypes) As String
            ...
            If String.IsNullOrWhiteSpace(fileName) Then
                ...
                fileName = "tumblr_" & idPart & InferExtension(mediaType)
            End If
        ...
        Private Function ResolveOutputPath() As String
            ...
            Return Path.Combine(Environment.CurrentDirectory, "TumblrDownloads")
        End Function
        ...
        Private Shared Function IsKnownMediaHost(ByVal host As String) As Boolean
            ...
            Dim h As String = host.ToLowerInvariant()
            Return h.Contains("media.tumblr.com") OrElse h = "tumblr.com" OrElse h = "www.tumblr.com" OrElse h.EndsWith(".tumblr.com")
        End Function
