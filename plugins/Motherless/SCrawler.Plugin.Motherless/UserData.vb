    ...
    Public Class UserData
        ...
        Public Sub GetMedia(ByVal Token As CancellationToken) Implements IPluginContentProvider.GetMedia
            ...
            If String.IsNullOrWhiteSpace(listingUrl) Then
                UserExists = False
                LogProvider?.Add("Motherless: unable to resolve listing URL.")
                Exit Sub
            End If
            ...
            Do While Not stopRequested AndAlso Not String.IsNullOrWhiteSpace(currentPage)
                ...
                Catch ex As WebException
                    ...
                    If status.HasValue AndAlso status.Value = HttpStatusCode.NotFound Then
                        UserExists = False
                        LogProvider?.Add("Motherless: listing page not found.")
                    Else
                        LogProvider?.Add(ex, String.Format("[Motherless.GetMedia] failed to load listing page: {0}", currentPage))
                    End If
                    Exit Do
            ...
            LogProvider?.Add(String.Format("Motherless: parsed {0} video page(s), collected {1} media item(s).", TempPostsList.Count, TempMediaList.Count))
        End Sub
        ...
        Public Sub Download(ByVal Token As CancellationToken) Implements IPluginContentProvider.Download
            ...
            For i As Integer = 0 To TempMediaList.Count - 1
                ...
                Catch ex As Exception
                    media.DownloadState = UserMediaStates.Missing
                    media.Attempts += 1
                    LogProvider?.Add(ex, String.Format("[Motherless.Download] {0}", media.URL))
                End Try
        ...
        Private Function TryResolveVideoMedia(ByVal videoUrl As String, ByVal token As CancellationToken) As IUserMedia
            ...
            Catch ex As Exception
                LogProvider?.Add(ex, String.Format("[Motherless.Resolve] failed to load video page: {0}", videoUrl))
                Return Nothing
            End Try
        ...
        Private Function ExtractVideoUrls(ByVal html As String, ByVal pageUrl As String) As List(Of String)
            ...
            For Each m As Match In Regex.Matches(html, "href=""(https?://(?:www\.)?motherless\.com/[A-F0-9]{6,8}(?:\?[^""<>]*)?)""", RegexOptions.IgnoreCase)
            ...
            For Each m As Match In Regex.Matches(html, "href=""(/([A-F0-9]{6,8})(?:\?[^""<>]*)?)""", RegexOptions.IgnoreCase)
        ...
        Private Function ExtractNextPageUrl(ByVal html As String, ByVal currentPage As String) As String
            ...
            If Not m.Success Then
                m = Regex.Match(html, "<a[^>]*href=""([^""<>]+)""[^>]*rel=""next""", RegexOptions.IgnoreCase)
            End If
            If Not m.Success Then
                m = Regex.Match(html, "<a[^>]*rel=""next""[^>]*href=""([^""<>]+)""", RegexOptions.IgnoreCase)
            End If
        ...
        Private Function ExtractBestDirectVideoUrl(ByVal html As String) As String
            ...
            Return ChooseBestVideoUrl(urls)
        End Function
        ...
        Private Shared Function ChooseBestVideoUrl(ByVal urls As List(Of String)) As String
            Dim bestUrl As String = String.Empty
            Dim bestQuality As Integer = -1

            For Each url As String In urls
                Dim quality As Integer = ExtractQualityScore(url)
                If quality > bestQuality Then
                    bestQuality = quality
                    bestUrl = url
                ElseIf bestUrl = String.Empty Then
                    bestUrl = url
                End If
            Next
        ...
        Private Shared Function ExtractQualityScore(ByVal url As String) As Integer
            If String.IsNullOrWhiteSpace(url) Then Return -1

            Dim m As Match = Regex.Match(url, "-([1-9][0-9]{2,3})p\.mp4", RegexOptions.IgnoreCase)
            If Not m.Success Then Return -1

            Dim q As Integer
            If Integer.TryParse(m.Groups(1).Value, q) Then Return q
            Return -1
        End Function

        Private Shared Function ExtractQualityTag(ByVal url As String) As String
            Dim q As Integer = ExtractQualityScore(url)
            If q > 0 Then Return q.ToString()
            Return "source"
        End Function
        ...
        Private Shared Function ExtractVideoId(ByVal videoUrl As String) As String
            ...
            If Uri.TryCreate(videoUrl, UriKind.Absolute, uri) Then
                Dim m As Match = Regex.Match(uri.AbsolutePath, "^/([A-F0-9]{6,8})$", RegexOptions.IgnoreCase)
                If m.Success Then Return m.Groups(1).Value
            End If

            Dim m2 As Match = Regex.Match(videoUrl, "/([A-F0-9]{6,8})(?:\?|$)", RegexOptions.IgnoreCase)
        ...
        Private Shared Function ExtractTitle(ByVal html As String) As String
            ...
            Dim m As Match = Regex.Match(html, "<title>([^<]+)</title>", RegexOptions.IgnoreCase)
            If Not m.Success Then Return String.Empty

            Dim t As String = WebUtility.HtmlDecode(m.Groups(1).Value).Trim()
            t = Regex.Replace(t, "\|\s*MOTHERLESS.*$", String.Empty, RegexOptions.IgnoreCase).Trim()
            Return t
        End Function
        ...
        Private Shared Function BuildVideoFileName(ByVal videoId As String, ByVal quality As String, ByVal directUrl As String) As String
            ...
            Dim fileName As String = String.Format("motherless_{0}_{1}.mp4", idPart, qualityPart)
        ...
        Private Shared Function IsVideoUrl(ByVal value As String) As Boolean
            ...
            If Uri.TryCreate(value, UriKind.Absolute, uri) Then
                Return Regex.IsMatch(uri.AbsolutePath, "^/[A-F0-9]{6,8}$", RegexOptions.IgnoreCase)
            End If

            Return Regex.IsMatch(value, "/[A-F0-9]{6,8}(?:\?|$)", RegexOptions.IgnoreCase)
        End Function

        Private Function ResolveOutputPath() As String
            If Not String.IsNullOrWhiteSpace(DataPath) Then Return DataPath
            Return Path.Combine(Environment.CurrentDirectory, "MotherlessDownloads")
        End Function
