...
Namespace VKPlugin
    ...
    Public Class SiteSettings
        ...
        <PropertyOption(ControlText:="Domain", ControlToolTip:="VK domain for URL checks"), PXML>
        Public ReadOnly Property Domain As PropertyValue

        <PropertyOption(ControlText:="Listing URL Pattern", ControlToolTip:="Use {0} for a profile or page key"), PXML>
        ...
        Public Function IsMyUser(ByVal UserURL As String) As ExchangeOptions Implements ISiteSettings.IsMyUser
            ...
            If IsPostPath(uri.AbsolutePath) OrElse IsDirectMediaUrl(uri) Then Return Nothing
            If Not IsProfilePath(uri.AbsolutePath) Then Return Nothing

            Dim key As String = ExtractUserKey(uri.AbsolutePath)
            If String.IsNullOrWhiteSpace(key) Then key = "vk"
        ...
        Private Shared Function IsPostPath(ByVal path As String) As Boolean
            ...
            If Regex.IsMatch(path, "^/(?:wall|photo|video|clip|doc|album)-?\d+_\d+/?$", RegexOptions.IgnoreCase) Then Return True
            If Regex.IsMatch(path, "^/story-?\d+_\d+/?$", RegexOptions.IgnoreCase) Then Return True
        ...
        Private Shared Function IsProfilePath(ByVal path As String) As Boolean
            ...
            If Regex.IsMatch(path, "^/(?:id\d+|club\d+|public\d+)/?$", RegexOptions.IgnoreCase) Then Return True
            If Regex.IsMatch(path, "^/[A-Za-z0-9_.-]{2,}/?$", RegexOptions.IgnoreCase) Then
                Dim key As String = path.Trim("/"c).ToLowerInvariant()
                Dim reserved As String() = {"feed", "video", "videos", "photo", "photos", "wall", "im", "friends", "groups", "music", "apps", "search", "login", "about", "privacy", "terms", "support"}
                For Each item As String In reserved
                    If key = item Then Return False
                Next
                Return True
            End If
        ...
        Private Shared Function ExtractUserKey(ByVal path As String) As String
            ...
            Dim parts() As String = path.Split({"/"c}, StringSplitOptions.RemoveEmptyEntries)
        ...
        Private Function IsDirectMediaUrl(ByVal uri As Uri) As Boolean
            ...
            Dim isVkCdn As Boolean = host.Contains("userapi.com") OrElse host.Contains("vk-cdn.net") OrElse host.Contains("vkuseraudio.net")
            If Not isVkCdn AndAlso Not HostMatchesDomain(host, GetDomain()) Then Return False

            Return Regex.IsMatch(uri.AbsolutePath,
                                 "\.(?:jpg|jpeg|png|gif|webp|bmp|mp4|webm|mov|m4v|mkv|avi|mp3|m4a|ogg|wav|flac|aac)$",
