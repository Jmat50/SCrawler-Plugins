    ...
    Public Class UserData
        ...
        Public Sub GetMedia(ByVal Token As CancellationToken) Implements IPluginContentProvider.GetMedia
            ...
            If String.IsNullOrWhiteSpace(listingUrl) Then
                UserExists = False
                LogProvider?.Add("DeviantArt: unable to resolve listing URL.")
                Exit Sub
            End If
            ...
            While pending.Count > 0
                ...
                Dim isRootPage As Boolean = String.Equals(pageKey, NormalizePageKey(listingUrl), StringComparison.OrdinalIgnoreCase)
                ...
                Catch ex As WebException
                    ...
                    If status.HasValue AndAlso status.Value = HttpStatusCode.NotFound Then
                        If isRootPage Then UserExists = False
                        LogProvider?.Add(String.Format("DeviantArt: page not found: {0}", pageUrl))
                    ElseIf status.HasValue AndAlso status.Value = HttpStatusCode.Gone Then
                        If isRootPage Then UserSuspended = True
                        LogProvider?.Add(String.Format("DeviantArt: page unavailable: {0}", pageUrl))
                    Else
                        LogProvider?.Add(ex, String.Format("[DeviantArt.GetMedia] failed to load page: {0}", pageUrl))
                    End If
                    Continue While
            ...
            LogProvider?.Add(String.Format("DeviantArt: parsed {0} page(s), collected {1} media item(s).", pagesParsed, TempMediaList.Count))
        End Sub
        ...
        Public Sub Download(ByVal Token As CancellationToken) Implements IPluginContentProvider.Download
            ...
            For i As Integer = 0 To TempMediaList.Count - 1
                ...
                Catch ex As Exception
                    media.DownloadState = UserMediaStates.Missing
                    media.Attempts += 1
                    LogProvider?.Add(ex, String.Format("[DeviantArt.Download] {0}", media.URL))
                End Try
        ...
        Public Sub DownloadSingleObject(ByVal Data As IDownloadableMedia, ByVal Token As CancellationToken) Implements IPluginContentProvider.DownloadSingleObject
            ...
            Else
                Try
                    ...
                    End If
                Catch ex As Exception
                    LogProvider?.Add(ex, String.Format("[DeviantArt.DownloadSingleObject] {0}", Data.URL))
                End Try
            End If
        ...
        Private Function NormalizeToAbsoluteUrl(ByVal value As String) As String
            ...
            If Regex.IsMatch(trimmed, "^[A-Za-z0-9_.-]+\.deviantart\.com(?:/.*)?$", RegexOptions.IgnoreCase) Then
            ...
            If Regex.IsMatch(trimmed, "^[A-Za-z0-9_.-]{2,}$", RegexOptions.IgnoreCase) Then
        ...
        Private Function DownloadPageText(ByVal url As String) As String
            ...
            Catch ex As WebException
                ...
                If status.HasValue AndAlso (status.Value = HttpStatusCode.Forbidden OrElse
                    ...
                    LogProvider?.Add(String.Format("DeviantArt: using fallback mirror for {0}", url))
        ...
        Private Function ExtractPostLinks(ByVal html As String, ByVal pageUrl As String) As List(Of String)
            ...
            Dim relativePattern As String =
                "(?:(?:href|data-href|data-url)\s*=\s*[""'])(/(?:[A-Za-z0-9_-]+/art/[^""'<>\s]+|deviation/\d+|art/[^""'<>\s]+))"
            ...
            Dim absolutePattern As String =
                "https?://(?:www\.)?deviantart\.com/(?:[A-Za-z0-9_-]+/art/[^""'<>\s]+|deviation/\d+|art/[^""'<>\s]+)"
        ...
        Private Function ExtractNextPageUrl(ByVal html As String, ByVal currentPage As String) As String
            ...
            If Not m.Success Then
                m = Regex.Match(html, "href=""([^""<>]*[?&](?:page|offset)=\d+[^""<>]*)""", RegexOptions.IgnoreCase)
            End If
            If Not m.Success Then
                m = Regex.Match(html, "href=""([^""<>]*/page/\d+(?:\?[^""<>]*)?)""", RegexOptions.IgnoreCase)
            End If
        ...
        Private Function ExtractDirectMediaUrls(ByVal html As String) As List(Of String)
            ...
            Dim pattern As String = "(?:https?:)?//[A-Za-z0-9\-._%/\?&=:+#~]+?\.(?:jpg|jpeg|png|gif|gifv|webp|bmp|mp4|webm|mov|m4v|mkv|avi|mp3|m4a|ogg|wav|flac|aac|m3u8)(?:\?[^""'\s<>)]+)?"
        ...
        Private Function NormalizeDirectMediaUrl(ByVal raw As String) As String
            ...
            Dim loweredPath As String = path.ToLowerInvariant()
            Dim isContentHost As Boolean = host.Contains("wixmp.com") OrElse host.Contains("sta.sh") OrElse loweredPath.Contains("/f/")
            If Not isContentHost AndAlso (host.Contains("deviantart.net") OrElse host.Contains("deviantart.com")) Then Return String.Empty

            If host.Equals("st.deviantart.net", StringComparison.OrdinalIgnoreCase) OrElse
               host.Equals("a.deviantart.net", StringComparison.OrdinalIgnoreCase) Then
                Return String.Empty
            End If

            Dim hasKnownExtension As Boolean = Regex.IsMatch(path,
                                                             "\.(?:jpg|jpeg|png|gif|gifv|webp|bmp|mp4|webm|mov|m4v|mkv|avi|mp3|m4a|ogg|wav|flac|aac|m3u8)$",
                                                             RegexOptions.IgnoreCase)
            Dim isDownloadEndpoint As Boolean = path.IndexOf("/download/", StringComparison.OrdinalIgnoreCase) >= 0
            If Not hasKnownExtension AndAlso Not isDownloadEndpoint Then Return String.Empty
        ...
        Private Shared Function ExtractPostIdFromUrl(ByVal value As String) As String
            ...
            If Uri.TryCreate(value, UriKind.Absolute, uri) Then
                Dim mPath As Match = Regex.Match(uri.AbsolutePath, "^/deviation/(\d+)(?:/|$)", RegexOptions.IgnoreCase)
                If mPath.Success Then Return mPath.Groups(1).Value

                mPath = Regex.Match(uri.AbsolutePath, "^/(?:[A-Za-z0-9_-]+/)?art/[^/?#]*-(\d{5,})(?:/|$)", RegexOptions.IgnoreCase)
                If mPath.Success Then Return mPath.Groups(1).Value

                Dim mQuery As Match = Regex.Match(uri.Query, "(?:^|[?&])deviationid=(\d+)", RegexOptions.IgnoreCase)
                If mQuery.Success Then Return mQuery.Groups(1).Value
            End If

            Dim m As Match = Regex.Match(value, "/deviation/(\d+)", RegexOptions.IgnoreCase)
            If m.Success Then Return m.Groups(1).Value

            m = Regex.Match(value, "/art/[^/?#]*-(\d{5,})", RegexOptions.IgnoreCase)
            If m.Success Then Return m.Groups(1).Value
        ...
        Private Shared Function BuildMediaFileName(ByVal directUrl As String, ByVal postId As String, ByVal mediaType As UserMediaTypes) As String
            ...
            If String.IsNullOrWhiteSpace(fileName) Then
                ...
                fileName = "deviantart_" & idPart & InferExtension(mediaType)
            End If
        ...
        Private Shared Function IsPostLikePath(ByVal path As String) As Boolean
            ...
            If Regex.IsMatch(path, "^/[A-Za-z0-9_-]+/art/[^/?#]+/?$", RegexOptions.IgnoreCase) Then Return True
            If Regex.IsMatch(path, "^/deviation/\d+/?$", RegexOptions.IgnoreCase) Then Return True
            If Regex.IsMatch(path, "^/art/[^/?#]+/?$", RegexOptions.IgnoreCase) Then Return True
        ...
        Private Function ResolveOutputPath() As String
            ...
            Return Path.Combine(Environment.CurrentDirectory, "DeviantArtDownloads")
        End Function
        ...
        Private Shared Function IsKnownMediaHost(ByVal host As String) As Boolean
            ...
            Dim h As String = host.ToLowerInvariant()
            Return h.Contains("wixmp.com") OrElse h.Contains("deviantart.net") OrElse h.Contains("deviantart.com") OrElse h.Contains("sta.sh")
        End Function
        ...
        Private Shared Function InferExtension(ByVal mediaType As UserMediaTypes) As String
            Select Case mediaType
                ...
                Case UserMediaTypes.GIF
                    Return ".gif"
                Case UserMediaTypes.Video, UserMediaTypes.VideoPre
                    Return ".mp4"
                Case UserMediaTypes.m3u8
                    Return ".m3u8"
