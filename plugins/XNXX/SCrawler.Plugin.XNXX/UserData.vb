    ...
    Public Class UserData
        ...
        Public Sub GetMedia(ByVal Token As CancellationToken) Implements IPluginContentProvider.GetMedia
            ...
            If String.IsNullOrWhiteSpace(listingUrl) Then
                UserExists = False
                LogProvider?.Add("XNXX: unable to resolve listing URL.")
                Exit Sub
            End If
            ...
            Do While Not stopRequested AndAlso Not String.IsNullOrWhiteSpace(currentPage)
                ...
                Catch ex As WebException
                    ...
                    If status.HasValue AndAlso status.Value = HttpStatusCode.NotFound Then
                        UserExists = False
                        LogProvider?.Add("XNXX: listing page not found.")
                    Else
                        LogProvider?.Add(ex, String.Format("[XNXX.GetMedia] failed to load listing page: {0}", currentPage))
                    End If
                    Exit Do
            ...
            LogProvider?.Add(String.Format("XNXX: parsed {0} video page(s), collected {1} media item(s).", TempPostsList.Count, TempMediaList.Count))
        End Sub
        ...
        Public Sub Download(ByVal Token As CancellationToken) Implements IPluginContentProvider.Download
            ...
            For i As Integer = 0 To TempMediaList.Count - 1
                ...
                Catch ex As Exception
                    media.DownloadState = UserMediaStates.Missing
                    media.Attempts += 1
                    LogProvider?.Add(ex, String.Format("[XNXX.Download] {0}", media.URL))
                End Try
        ...
        Private Function TryResolveVideoMedia(ByVal videoUrl As String, ByVal token As CancellationToken) As IUserMedia
            ...
            Catch ex As Exception
                LogProvider?.Add(ex, String.Format("[XNXX.Resolve] failed to load video page: {0}", videoUrl))
                Return Nothing
            End Try

            Dim directUrl As String = ExtractBestDirectVideoUrl(html)
            If String.IsNullOrWhiteSpace(directUrl) Then
                LogProvider?.Add(String.Format("XNXX: no MP4 source found for {0}", videoUrl))
                Return Nothing
            End If
        ...
        Private Function ExtractVideoUrls(ByVal html As String, ByVal pageUrl As String) As List(Of String)
            ...
            For Each m As Match In Regex.Matches(html, "href=""(/video-[^""<>?#]+[^""<>]*)""", RegexOptions.IgnoreCase)
            ...
            For Each m As Match In Regex.Matches(html, "href=""(https?://(?:www\.)?xnxx\.com/video-[^""<>?#]+[^""<>]*)""", RegexOptions.IgnoreCase)
        ...
        Private Function ExtractNextPageUrl(ByVal html As String, ByVal currentPage As String) As String
            ...
            If Not m.Success Then
                m = Regex.Match(html, "<a[^>]*href=""([^""<>]+)""[^>]*class=""[^""]*\bnext\b[^""]*""", RegexOptions.IgnoreCase)
            End If
            If Not m.Success Then
                m = Regex.Match(html, "<a[^>]*class=""[^""]*\bnext\b[^""]*""[^>]*href=""([^""<>]+)""", RegexOptions.IgnoreCase)
            End If
        ...
        Private Shared Function ExtractDirectVideoUrls(ByVal html As String) As List(Of String)
            ...
            For Each m As Match In Regex.Matches(normalized, """contentUrl""\s*:\s*""(https?://[^""\s]+\.mp4[^""\s]*)""", RegexOptions.IgnoreCase)
                Dim u As String = DecodeEncodedUrl(m.Groups(1).Value)
                If IsValidAbsoluteUrl(u) AndAlso unique.Add(u) Then result.Add(u)
            Next

            For Each m As Match In Regex.Matches(normalized, "https?://[^""'\s<]+\.mp4[^""'\s<]*", RegexOptions.IgnoreCase)
        ...
        Private Shared Function ExtractVideoId(ByVal videoUrl As String) As String
            ...
            Dim m As Match = Regex.Match(videoUrl, "/video-([a-z0-9]+)(?:/|$)", RegexOptions.IgnoreCase)
        ...
        Private Shared Function BuildVideoFileName(ByVal videoId As String, ByVal quality As String, ByVal directUrl As String) As String
            ...
            Dim fileName As String = String.Format("xnxx_{0}_{1}.mp4", idPart, qualityPart)
        ...
        Private Shared Function IsVideoUrl(ByVal value As String) As Boolean
            ...
            If Uri.TryCreate(value, UriKind.Absolute, uri) Then
                Return uri.AbsolutePath.StartsWith(VideoPathPrefix, StringComparison.OrdinalIgnoreCase) OrElse
                       uri.AbsolutePath.StartsWith("/video/", StringComparison.OrdinalIgnoreCase)
            End If

            Return value.IndexOf(VideoPathPrefix, StringComparison.OrdinalIgnoreCase) >= 0
        End Function

        Private Function ResolveOutputPath() As String
            If Not String.IsNullOrWhiteSpace(DataPath) Then Return DataPath
            Return Path.Combine(Environment.CurrentDirectory, "XNXXDownloads")
        End Function
