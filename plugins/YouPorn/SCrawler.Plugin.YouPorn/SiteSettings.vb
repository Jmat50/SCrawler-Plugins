...
Namespace YouPornPlugin
    ...
    Public Class SiteSettings
        ...
        <PropertyOption(ControlText:="Domain", ControlToolTip:="YouPorn domain for URL checks"), PXML>
        Public ReadOnly Property Domain As PropertyValue

        <PropertyOption(ControlText:="Listing URL Pattern", ControlToolTip:="Use {0} for the creator/page key"), PXML>
        ...
        Public Function IsMyUser(ByVal UserURL As String) As ExchangeOptions Implements ISiteSettings.IsMyUser
            ...
            If Not IsSupportedListingPath(uri.AbsolutePath) Then Return Nothing

            Dim key As String = ExtractUserKey(uri.AbsolutePath)
            If String.IsNullOrWhiteSpace(key) Then key = "youporn"
        ...
        Public Function IsMyImageVideo(ByVal URL As String) As ExchangeOptions Implements ISiteSettings.IsMyImageVideo
            ...
            Dim targetDomain As String = GetDomain()
            If targetDomain <> String.Empty AndAlso host.EndsWith(targetDomain, StringComparison.OrdinalIgnoreCase) Then
                Dim isVideoPage As Boolean = path.StartsWith(WatchPathPrefix, StringComparison.OrdinalIgnoreCase) OrElse
                                         path.StartsWith("/embed/", StringComparison.OrdinalIgnoreCase) OrElse
            ...
            If host.IndexOf("ypncdn.com", StringComparison.OrdinalIgnoreCase) >= 0 AndAlso
               path.IndexOf(".mp4", StringComparison.OrdinalIgnoreCase) >= 0 Then
                Return New ExchangeOptions With {
                    .SiteName = Site,
                    .HostKey = PluginKey,
                    .UserName = String.Empty,
        ...
        Private Shared Function IsVideoLikePath(ByVal absolutePath As String) As Boolean
            ...
            Return absolutePath.StartsWith(WatchPathPrefix, StringComparison.OrdinalIgnoreCase) OrElse
                   absolutePath.StartsWith("/embed/", StringComparison.OrdinalIgnoreCase) OrElse
                   absolutePath.StartsWith("/media/", StringComparison.OrdinalIgnoreCase)
        End Function
        ...
        Private Function NormalizeToAbsoluteUrl(ByVal value As String) As String
            ...
            If Uri.TryCreate(trimmed, UriKind.Absolute, absolute) Then
                Dim targetDomain As String = GetDomain()
                If targetDomain <> String.Empty AndAlso absolute.Host.EndsWith(targetDomain, StringComparison.OrdinalIgnoreCase) Then
