    ...
    Public Class UserData
        ...
        Public Sub GetMedia(ByVal Token As CancellationToken) Implements IPluginContentProvider.GetMedia
            ...
            If String.IsNullOrWhiteSpace(listingUrl) Then
                UserExists = False
                LogProvider?.Add("YouPorn: unable to resolve a supported listing URL. Try /channel/{name}/, /pornstar/{name}/, /category/{name}/, /porntags/{name}/, /browse/{mode}/, or ranking pages like /recommended/.")
                Exit Sub
            End If

            If IsWatchUrl(listingUrl) Then
            ...
            Do While Not stopRequested AndAlso Not String.IsNullOrWhiteSpace(currentPage)
                ...
                Catch ex As WebException
                    ...
                    If status.HasValue AndAlso status.Value = HttpStatusCode.NotFound Then
                        UserExists = False
                        LogProvider?.Add("YouPorn: listing page not found.")
                    Else
                        LogProvider?.Add(ex, String.Format("[YouPorn.GetMedia] failed to load listing page: {0}", currentPage))
                    End If
                    Exit Do
                ...
                Dim watchUrls As List(Of String) = ExtractWatchUrls(html, currentPage)
                ...
                If watchUrls.Count = 0 AndAlso String.IsNullOrWhiteSpace(nextPage) Then
                    LogProvider?.Add(String.Format("YouPorn: no video links were found on {0}. The supplied page is likely not a supported public listing route.", currentPage))
                End If
            ...
            LogProvider?.Add(String.Format("YouPorn: parsed {0} video page(s), collected {1} media item(s).", TempPostsList.Count, TempMediaList.Count))
        End Sub
        ...
        Public Sub Download(ByVal Token As CancellationToken) Implements IPluginContentProvider.Download
            ...
            For i As Integer = 0 To TempMediaList.Count - 1
                ...
                Catch ex As Exception
                    media.DownloadState = UserMediaStates.Missing
                    media.Attempts += 1
                    LogProvider?.Add(ex, String.Format("[YouPorn.Download] {0}", media.URL))
                End Try
        ...
        Private Function TryResolveVideoMedia(ByVal watchUrl As String, ByVal token As CancellationToken) As IUserMedia
            token.ThrowIfCancellationRequested()
            Thrower?.ThrowAny()

            Dim html As String = Nothing
            Try
                html = DownloadString(watchUrl, "text/html,*/*")
            Catch ex As Exception
                LogProvider?.Add(ex, String.Format("[YouPorn.Resolve] failed to load watch page: {0}", watchUrl))
                Return Nothing
            End Try

            Dim mediaEndpoint As String = ExtractMp4MediaEndpoint(html)
            If String.IsNullOrWhiteSpace(mediaEndpoint) Then
                LogProvider?.Add(String.Format("YouPorn: no MP4 media endpoint found for {0}", watchUrl))
                Return Nothing
            End If
            ...
            Catch ex As Exception
                LogProvider?.Add(ex, String.Format("[YouPorn.Resolve] failed to load media endpoint: {0}", mediaEndpoint))
                Return Nothing
            End Try

            Dim serializer As JavaScriptSerializer = CreateSerializer()
            Dim sources As List(Of YouPornMediaSource) = Nothing

            Try
                sources = serializer.Deserialize(Of List(Of YouPornMediaSource))(mediaJson)
            Catch ex As Exception
                LogProvider?.Add(ex, "[YouPorn.Resolve] failed to parse MP4 source list")
                Return Nothing
            End Try

            If sources Is Nothing OrElse sources.Count = 0 Then Return Nothing

            Dim selected As YouPornMediaSource = ChooseBestSource(sources)
            ...
            Dim watchId As String = ExtractWatchId(watchUrl)
        ...
        Private Function NormalizeToAbsoluteUrl(ByVal value As String) As String
            ...
            If Uri.TryCreate(trimmed, UriKind.Absolute, absolute) Then
                If absolute.Host.EndsWith(GetDomainOnly(), StringComparison.OrdinalIgnoreCase) Then
                    Dim normalizedAbsolute As String = absolute.GetLeftPart(UriPartial.Path) & absolute.Query
                    If IsWatchUrl(normalizedAbsolute) OrElse IsSupportedListingPath(absolute.AbsolutePath) Then
        ...
        Private Function ExtractWatchUrls(ByVal html As String, ByVal pageUrl As String) As List(Of String)
            ...
            For Each m As Match In Regex.Matches(html, "href=""(/watch/\d+/[^""""<>]*)""", RegexOptions.IgnoreCase)
        ...
        Private Shared Function ExtractWatchId(ByVal watchUrl As String) As String
            If String.IsNullOrWhiteSpace(watchUrl) Then Return String.Empty

            Dim m As Match = Regex.Match(watchUrl, "/watch/(\d+)/", RegexOptions.IgnoreCase)
            If m.Success Then Return m.Groups(1).Value

            m = Regex.Match(watchUrl, "/watch/(\d+)", RegexOptions.IgnoreCase)
            If m.Success Then Return m.Groups(1).Value
        ...
        Private Shared Function ChooseBestSource(ByVal sources As List(Of YouPornMediaSource)) As YouPornMediaSource
            If sources Is Nothing OrElse sources.Count = 0 Then Return Nothing

            Dim chosen As YouPornMediaSource = sources.Find(Function(s) s IsNot Nothing AndAlso s.defaultQuality AndAlso Not String.IsNullOrWhiteSpace(s.videoUrl))
            If chosen IsNot Nothing Then Return chosen

            Dim best As YouPornMediaSource = Nothing
            Dim bestQuality As Integer = -1

            For Each s As YouPornMediaSource In sources
        ...
        Private Shared Function BuildVideoFileName(ByVal watchId As String, ByVal quality As String, ByVal directUrl As String) As String
            Dim idPart As String = If(String.IsNullOrWhiteSpace(watchId), "video", watchId)
            Dim qualityPart As String = If(String.IsNullOrWhiteSpace(quality), "source", quality)

            Dim fileName As String = String.Format("youporn_{0}_{1}.mp4", idPart, qualityPart)
        ...
        Private Shared Function IsWatchUrl(ByVal value As String) As Boolean
            If String.IsNullOrWhiteSpace(value) Then Return False

            Dim uri As Uri = Nothing
            If Uri.TryCreate(value, UriKind.Absolute, uri) Then
                Return uri.AbsolutePath.StartsWith(WatchPathPrefix, StringComparison.OrdinalIgnoreCase)
            End If

            Return value.IndexOf(WatchPathPrefix, StringComparison.OrdinalIgnoreCase) >= 0
        End Function

        Private Function ResolveOutputPath() As String
            If Not String.IsNullOrWhiteSpace(DataPath) Then Return DataPath
            Return Path.Combine(Environment.CurrentDirectory, "YouPornDownloads")
        End Function
        ...
        Private NotInheritable Class YouPornMediaSource
